...
app.post('/api/postMessage', async (req, res) => {
  ...
  runClient(JSON.stringify(new message.Message('PUBLISH', jsonData.topic, jsonData.message, jsonData.key)))
  res.status(200).end()
})
...
function isJsonResponseError(rawString) {
  ...
  return parsedJson !== undefined && parsedJson.content !== undefined && parsedJson === 'ERROR'
}
